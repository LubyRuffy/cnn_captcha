...
lst = []
for line in open('../../../cnn_captcha/tmp/4nums_test_lst.txt', 'r'):
    lst.append('../../../cnn_captcha/tmp/' + line.strip())
...
for sam_idx in range(len(lst)):
    ...
    if img0 != None:
        ...
        print(pred_y0.shape)
        print(pred_y0)
        idx1 = np.argmax(pred_y0[0, 0:10])
        idx2 = np.argmax(pred_y0[0, 10:20])
        idx3 = np.argmax(pred_y0[0, 20:30])
        idx4 = np.argmax(pred_y0[0, 30:40])
